...
# %%
#2
animals.dtype  # preuve inhomogénéité des valeurs

# %%
#3
df = pd.DataFrame(columns=['names','speed','lifespan'], data=animals)
df
...
# %%
#6
df.iloc[2,0] = 'gray elephant'

x=df.loc[2,'names']
x, type(x) # encore une 'str'

# %% scrolled=true
#7
df.dtypes

# je n'ai pas saisi ce qu'il fallait tirer de cela, 
# à part que panda dit 'mixed data types in columns' avec le type 'object'

# %%
#8
df.to_numpy().dtype # donc 'str'

# %%
#9
df['speed'] = df['speed'].astype(dtype='float')
df['lifespan'] = df['lifespan'].astype(dtype='float')
df[['speed','lifespan']].values
...
#   alors que `concat` ne considère que les index (à bon ?)
#
...
# %%
# votre code
df4 = pd.concat([df1,df2['grade']],axis=1)
...
# %% tags=["level_intermediate"]
# votre code
df4i = pd.concat([df1i,df2i],axis=1)

df5i=pd.concat([df4i,df3i],axis=0)
