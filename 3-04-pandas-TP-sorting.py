...
# %%
plt.plot(df.index,df.loc[:,'Age'],'rs') # utiliser .loc
...
# %%
df2[df2.loc[:,'Age'].notna()].tail(3)

# %%
plt.plot(df.loc[:,'Age'])

# %% {"scrolled": true}
plt.plot(df2.loc[:,'Age'])
...
# %% [markdown]
# les passagers dont l'âge n'est pas renseigné sont en fin de dataframe

# %% {"scrolled": true}
#6
df2 = df2.sort_values(by=['Age','Fare'],na_position='first')
df2

# %%
#7
df2 = df[df['Age'].notna()].sort_values(by=['Age','Fare'])
df2
...
# %%
#1
tab = np.random.randint(0,101,(4,5))
df = pd.DataFrame(columns=['a','b','c','d','e'], index=['un','deux','trois','quatre'],data=tab)
...
# %%
#3
# l'axe détermine le type d'objets qu'on bouge : lignes ou colonnes
df.sort_values(axis=1, by='trois', inplace=True)
